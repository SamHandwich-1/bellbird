...
function usage(): never {
  console.error(
    [
      ...
      `      --prompt <file> --fragment <file> --model <${MODELS.join('|')}>`,
      '',
      '  Matrix:',
      '    npm run prompt-harness -- \\',
      `      --prompt-dir <dir> --fragment-dir <dir> --model <${MODELS.join('|')}>`,
      '',
      '  Flags:',
...
// One-level recursion with a symmetric ALLOW-LIST at both top level and
// sub-folders: include only files matching `phase-*.{md,txt}`. A README, a
// CHANGELOG, or a scenario-folder sidecar like `expected.md` (reviewer checklist)
// is structurally excluded. A deny-list would not survive future sidecars; the
// pattern lock does.
function isPhaseInputFile(name: string): boolean {
  const ext = extname(name).toLowerCase();
  if (ext !== '.md' && ext !== '.txt') return false;
  return name.toLowerCase().startsWith('phase-');
}
...
function listInputFiles(dir: string): string[] {
  ...
  for (const name of readdirSync(dir)) {
    ...
    if (stat.isFile()) {
      if (isPhaseInputFile(name)) out.push(full);
      continue;
    }
    if (stat.isDirectory()) {
      for (const inner of readdirSync(full)) {
        ...
        if (isPhaseInputFile(inner)) out.push(innerFull);
      }
    }
  }
  return out.sort();
