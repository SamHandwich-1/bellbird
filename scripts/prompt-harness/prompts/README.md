...
- `phase-4-fable.md` — Fable 5 evaluation scratch, not an A/B variant. Literal copy of the promoted D prompt from `lib/ai/prompts/phase-4-adjudication.ts` with the model self-label ("— Opus 4.8") removed so the prompt is model-agnostic. Exists to re-verify the three boundary poles (uranium → PROCEED, mall-REIT → STRESS_TEST, Veridian → DISCARD) on `claude-fable-5` before any migration — a model swap voids prompt verification the same way a prompt edit does. Run with `--model fable --max-tokens 16000` (Fable's always-on thinking counts toward max-tokens; the default 4096 risks truncating the verdict).
